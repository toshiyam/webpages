...
// index.html側には値を固定文言として書かない。#offlineExplain の表示文言は
// renderObserve() がこの定数から都度組み立てるため、表示とロジックが
// 乖離することはない（issue #19レビュー対応: 表示値をJS側と分離して
// 固定しないこと）。
// なお、mortality.jsのmortalityChance()はage>=100で必ず1（=Math.random()の
// 返り値がどうであれ確実に死亡）を返し、年齢はsimulateYearで毎年必ず+1
// されるため、誕生（age=0）から数えて約100年以内に必ず死亡する。つまり
// この上限（300）に達する前に必ず人生が終わるため、「上限に達したため
// 打ち切った」という通知はこのゲームの死亡則の下では実際には発生しえない
// （到達不能なコードになるため、catchUpOffline側にはあえて実装していない）。
...
// desc: 「速度変更の意味を短く表示する」（issue #19）ための、1年あたりの
// 実時間を短い日本語で示す文言。yearMsから動的に算出せず固定文言にしている
// のは、四捨五入や単位（秒/ミリ秒）の表記揺れを避けるため。
var SPEEDS = [
  { id: 'slow', label: '遅い', yearMs: 8000, desc: '1年 ≈ 8秒' },
  { id: 'normal', label: '標準', yearMs: 3000, desc: '1年 ≈ 3秒' },
  { id: 'fast', label: '速い', yearMs: 900, desc: '1年 ≈ 0.9秒' }
];
...
// issue#136: docs/save-compat-strategy.md 5.1節「未来schema(save_too_new)
// 検出時は主データ・backup・corruptedのいずれも一切書き換えない読み取り
// 専用の起動停止が必須」「起動停止（操作そのものをブロックする）を
// 採用する」。新UI側(web-ui-src/src/save/storage.ts)は既にこの契約を
// 満たしていたが、本UIのsanitizeLoaded()は未来schemaを「破損」と区別せず
// freshState()へフォールバックしてゲームを続行させ、その後のsaveState()
// でfreshState()の内容をSTORAGE_KEYへ上書きしてしまっていた（未来schema
// のデータを黙って破棄・上書きする不具合）。このフラグが立っている間、
// init()は通常のゲームUIを表示せず起動停止画面のみを表示し、イベント
// 購読・renderAll()等を一切行わない（init()参照）。saveState()側にも
// 同じフラグでの拒否を残し、万一この起動停止を経由しない呼び出し経路が
// 生じても書き込みを二重に防ぐ（本UIのlocalStorage書き込み箇所は
// saveState()のこの1箇所のみ、backup/corrupted相当のキーは持たない）。
var saveBlockedFutureSchema = false;
...
function loadState() {
  ...
  // issue#136: 未来schema(schemaVersionが現在のSCHEMA_VERSIONより大きい)は
  // 「破損」とは独立に判定し、sanitizeLoaded()の一般的な破損経路（新しく
  // 開始する＝この端末の保存内容を次のsaveState()で上書きしてしまう）へは
  // 進ませない。docs/save-compat-strategy.md 5.1節に従い起動停止するため、
  // ここではフラグを立てて返すだけに留め、実際の起動停止画面表示は
  // init()側の責務とする（addBanner()は使わない。起動停止時は.app自体を
  // 非表示にするため、通常のバナー領域はそもそも画面に出ない）。
  if (typeof raw.schemaVersion === 'number' && raw.schemaVersion > SCHEMA_VERSION) {
    saveBlockedFutureSchema = true;
    lastSaveOk = false;
    return freshState();
  }
...
function saveState() {
  // issue#136: 未来schemaを検出したセッションでは、この関数が唯一の
  // localStorage書き込み箇所であるため、ここで一括して書き込みを禁止する
  // （新規プレイの継続自体は妨げないが、その内容はこの端末に一切保存され
  // ない一時プレイとして扱う。docs/save-compat-strategy.md 5.1節）。
  if (saveBlockedFutureSchema) {
    renderSaveStatus();
    return;
  }
...
function renderObserve() {
  ...
  // 選択中の速度が具体的に何を意味するか（1年あたりの実時間）を短く表示する
  // （issue #19: 「速度変更の意味を短く表示する」要件への対応）。
  var speed = SPEEDS[state.speedIndex];
  document.getElementById('speedDesc').textContent = speed.desc;
  // catchUpOffline()は不在時間(ms)を「選択中の速度のyearMs」で年数に換算する
  // ため、速い速度を選んだまま短時間離れただけでも大量の年数が処理されうる
  // （例: 速い=1年≈0.9秒なら、約90秒の不在で約100年、通常はその途中で死亡
  // する）。この「選択中の速度がオフライン換算にも使われる」という事実を
  // 明示しないと、「勝手に進みすぎた」という事故感の主因を説明できていない
  // ことになる（issue #19レビュー対応）。数値(MAX_OFFLINE_YEARS)は
  // index.html側に固定文言として持たず、ここで組み立てることで表示と
  // 実際の上限値が乖離しないようにする。
  document.getElementById('offlineExplain').textContent =
    '不在時間もこの速度（' + speed.label + '）で換算されるため、速い速度のまま長く離れると一気に年月が進みます。' +
    '不在中は最大' + MAX_OFFLINE_YEARS + '年分まで自動処理し、死亡・特殊結末に至った場合はそこで停止します。';
}
...
async function init() {
  data = await loadData();
  state = loadState();
  // issue#136: docs/save-compat-strategy.md 5.1節「検出時は主データ・backup・
  // corruptedのいずれも一切書き換えない読み取り専用の起動停止」。
  // saveState()自体もsaveBlockedFutureSchema===trueの間は書き込みを拒否する
  // （多重防御）が、それに加えてここで操作そのものをブロックする——プレイ
  // ヤー操作を一切受け付けない起動停止画面を表示し、通常のイベント購読・
  // renderAll()・catchUpOffline()（いずれもゲーム状態の変更や後続の
  // saveState()呼び出しにつながりうる）を一切行わずinit()を終了する。
  if (saveBlockedFutureSchema) {
    document.querySelector('.app').hidden = true;
    var halt = document.getElementById('haltScreen');
    halt.textContent = '保存データが現在のアプリより新しい形式のため、データを保護する目的で起動を停止しています。アプリを更新すると、このメッセージは表示されなくなります。';
    halt.hidden = false;
    return;
  }
  catchUpOffline();
